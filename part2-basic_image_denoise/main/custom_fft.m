function Y = custom_fft(X, manual_flag, plot_flag)
...
if manual_flag
    ...
    x1 = round(size(X1_shift, 1)/2 - 0.02*size(X1_shift, 1));
    x2 = round(size(X1_shift, 1)/2 + 0.02*size(X1_shift, 1));
    y1 = 1;
    y2 = round(0.45*size(X1_shift, 2));
    X1_shift(round(y1):round(y2), round(x1):round(x2)) = 0;
    X1_shift(size(X1_shift, 2)-[round(y2):-1:round(y1)], round(x1):round(x2)) = 0;
end
